...
struct interp_ctx* interp_new(struct allocator *alloc) {
    ...
    env_bind(alloc, ret->top_env, make_symbol(ret->alloc, "="), make_builtin2(ret->alloc, &builtin_numeric_equals));
    ...
    env_bind(alloc, ret->top_env, make_symbol(ret->alloc, "eqv?"), make_builtin2(ret->alloc, &builtin_eqv));
    env_bind(alloc, ret->top_env, make_symbol(ret->alloc, "equal?"), make_builtin2(ret->alloc, &builtin_equal));
    ...
    env_bind(alloc, ret->top_env, make_symbol(ret->alloc, "symbol=?"), make_builtin2(ret->alloc, &builtin_symbol_eq));
